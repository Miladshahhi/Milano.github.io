...
class Hediye:
    ...
    def nitrxgen(self):
        on = requests.get('http://www.nitrxgen.net/md5db/' + urllib.parse.quote(self.net)).text
        if len(on) != 0:
            print(Style.BRIGHT + '   Value Found  For -->  :', Fore.GREEN, self.net, Fore.WHITE, ':', Fore.RED, on,
                  Style.RESET_ALL)
        else:
            self.hashtoolkit()
    ...
    def hashtoolkit(self):
        words = set()
        on = requests.get('https://hashtoolkit.com/decrypt-md5-hash/' + urllib.parse.quote(self.net),
                          headers={'User-agent': 'Mozilla/11.0'}).text
                    ...
                    for word in line.replace('.', ' ').replace(
                        ...
                        if word not in words:
                            ...
                            if md5(word.encode()).hexdigest() == self.net:
                                print(Style.RESET_ALL, Style.BRIGHT + '   Value Found  For -->  :', Fore.GREEN,
                                      self.net, Fore.WHITE, ':', Fore.RED, word, Style.RESET_ALL)
                                return
        self.search()
    ...
    def search(self):
        ...
        on = requests.get('https://www1.search-results.com/web?q=' + urllib.parse.quote(self.net)).text
        goo = requests.get('https://www.google.com/search?q=' + urllib.parse.quote(self.net)).text
        ya = requests.get('https://search.yahoo.com/search?p=' + urllib.parse.quote(self.net)).text
        bing = requests.get('https://www.bing.com/search?q=' + urllib.parse.quote(self.net)).text
        ask = requests.get('https://de.ask.com/web?q=' + urllib.parse.quote(self.net)).text
    ...
    def words(self):
        ...
        if len(self.hash) == 32:
            ...
            print(Style.BRIGHT + '\ncheck thisinput this input The Hash !!! ¯\_(ツ)_/¯\n', '\n', self.hash)
            sys.exit()
            ...
            for dic in data:
                ...
                if self.value(dic.encode()).hexdigest() == self.hash:
                    print(Style.RESET_ALL, Style.BRIGHT, '\nValue Found  For -->        :', Fore.GREEN, self.hash, '\n',
                          Fore.WHITE, '	     Key -->        :', Fore.RED, dic, Fore.WHITE, '\nTested', Fore.RED, keys,
                          Fore.WHITE, 'word in', Fore.RED, self.dict, Fore.WHITE, 'elapsed time', Fore.CYAN,
                          end - start, '\n', Style.RESET_ALL)
                    sys.exit()

                else:
                    pass
            print(Style.RESET_ALL, Style.BRIGHT + '\nValue Not Found  For -->       :', Fore.RED, self.hash,
                  Fore.WHITE, '\nTested', Fore.RED, keys, Fore.WHITE, 'word in', Fore.RED, self.dict, Fore.WHITE,
                  'elapsed time', Fore.GREEN, end - start, '\n', Style.RESET_ALL)
...
{03}{0} -u{1} Check For update '''.format(__appname__ + '.py', __yes__, __kir__, __res__, __size__))
    ...
    if x.update:
        cw.banner()
        on = requests.get(__github__).text
        if __version__ in on:
            print(Style.BRIGHT, Fore.GREEN, 'You are using the latest version!')
        else:
            with open(__file__, mode='w+') as data:
                data.write(on)
            print(Style.BRIGHT, Fore.GREEN, 'Update completed! Please run the script again.')

    if not any([x.key, x.dict, x.net, x.update]):
        print(parser.epilog)
